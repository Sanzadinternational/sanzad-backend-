...
export const downloadVoucher = async (req: Request, res: Response) => {
  try {
    ...
    // Add voucher content (customize as needed)
    doc.fontSize(20).text('Voucher', { align: 'center' });
    doc.moveDown();

    doc.fontSize(14).text(`Voucher ID: ${voucher.id}`);
    doc.text(`Name: ${voucher.name || 'N/A'}`);
    doc.text(`Amount: ₹${voucher.amount || 0}`);
    doc.text(`Date: ${voucher.date || new Date().toISOString()}`);
